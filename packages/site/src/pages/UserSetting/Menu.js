...
const Menu = ({ tab, setTab }) => {
  return (
    <Wrapper>
      ...
      <Item selected={tab === NOTIFICATION} onClick={() => setTab(NOTIFICATION)}>
        <ItemImage src="/imgs/bell.svg" />
        <ItemText>Notification</ItemText>
      </Item>
      <Item selected={tab === LINKED_ADDRESSES} onClick={() => setTab(LINKED_ADDRESSES)}>
        <ItemImage src="/imgs/connected.svg" />
        <ItemText>Linked addresses</ItemText>
      </Item>
    </Wrapper>
  )
}
