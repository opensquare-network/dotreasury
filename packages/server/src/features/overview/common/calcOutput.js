...
async function calcOutput(
  ...
  const proposalSpent = spentProposals.reduce(
    (result, { value, symbolPrice }) => ({
      ...
      fiatValue: addUsdtValue(result.fiatValue, value, symbolPrice).toNumber(),
    }),
    { value: 0, fiatValue: 0 },
  );

  const tipSpent = tips.reduce(
    (result, { state, medianValue, symbolPrice }) => {
  ...
  const bountySpent = bounties.reduce(
    (result, { meta: { status, value }, symbolPrice }) => {
